    ...
    public partial class Jobcard : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Page.Request.QueryString.Count > 0 && !IsPostBack)
            {
                ...
                SampleInformationTableHeadereRow.Cells.Add(new TableHeaderCell { Text = "Bottle Information ", Height = Unit.Pixel(20) });
                SampleInformationTableHeadereRow.Cells.Add(new TableHeaderCell { Text = "Point Number", Height = Unit.Pixel(20) });
                SampleInformationTableHeadereRow.Cells.Add(new TableHeaderCell { Text = "Collected Date", Height = Unit.Pixel(20) });
                ...
                if (listSampleDetail.Count > 0)
                {
                    foreach (var result in listSampleDetail)
                    {
                        ...
                        {
                            tableBodyRow.Cells.Add(new TableCell { Text = result.SampleInformation });
                            tableBodyRow.Cells.Add(new TableCell { Text = result.TracingPoint });
                ...
                labNumber.Text = objJobInfo.JobNumber;
                ...
                //printTestList.Items.Clear();
                //printTestList.Items.Add(new ListItem() { Text = "All", Value = "0" });
                //foreach (
                //    var objTestParameterInfo in
                //        testResult.Select(result => objDbController.GetTestParameterInfo(result.TestParameterInfoId)))
                //{
                //    printTestList.Items.Add(new ListItem()
                //                                {
                //                                    Text = objTestParameterInfo.AccreditedTestParameter,
                //                                    Value = objTestParameterInfo.TestParameterId.ToString()
                //                                });
                //}
            ...
            foreach (var result in testResult)
            {
                ...
                using (var tableBodyRow = new TableRow { TableSection = TableRowSection.TableBody })
                {
                    ...
                    tb1.Text = result.Result != 0 ? result.Result.ToString("0.00", CultureInfo.InvariantCulture) : "";
                    ...
                    DropDownList list = new DropDownList { ID = "Action" + result.TestResultId };
                    list.Items.Add(new ListItem { Text = "Pass" });
                    list.Items.Add(new ListItem { Text = "Fail" });
                    list.Items.Add(new ListItem { Text = "Cancel" });
                    var tableCell3 = new TableCell();
                    tableCell3.Controls.Add(list);
                    tableBodyRow.Cells.Add(tableCell3);
                    ...
                    //!result.IsValueEntered &&
                    if (((!result.IsLocked || result.TmpChangeBy == Int32.Parse(Session["userId"].ToString()))))
                    {
                        if (!IsPostBack)
                        {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            foreach (var testResult in listUpdatedResult)
            {
                if (testResult.IsValueModify)// && !testResult.IsValueEntered)
                {
                    ...
                    DropDownList list = (DropDownList)pnl1.FindControl("Action" + testResult.TestResultId);
                    testResult.Status = list.SelectedValue;
            ...
            if (isComplete)
            {
                objActiveTestInfo.CurrentStatus = "Completed";
        ...
        protected void PrintCOA_Click(object sender, EventArgs e)
        {
            Session["ActionType"] = "Print";
            ClientScript.RegisterStartupScript(GetType(), "onclick",
                                               string.Format(
                                                   "<script language=javascript>window.open('{0}','PrintMe','height=300px,width=300px,scrollbars=1');</script>",
                                                   string.Format("COAPrintable.aspx?atid={0}&tid={1}",
                                                                 Page.Request.QueryString["atid"], selectedTest)));
        }

        protected void ECardCOA_Click(object sender, EventArgs e)
        {
            CustomerInfo objCustomerInof = objDbController.GetCustomerData(ClientName.Text);
            ContactInfo objContactInfo = objDbController.GetContactInfo(objCustomerInof.CustID);
            Session["ToEmail"] = objContactInfo.Email;
            ...
            ClientScript.RegisterStartupScript(GetType(), "onclick",
                                               string.Format(
                                                   "<script language=javascript>window.open('{0}','PrintMe','height=300px,width=300px,scrollbars=1');</script>",
                                                   string.Format("COAEmail.aspx?atid={0}&tid={1}",
                                                                 Page.Request.QueryString["atid"], selectedTest)));

            ActiveTestsInfo objActiveTestInfo =
              Global.ObjDbController.GetActiveTestInfo(Int32.Parse(Page.Request.QueryString["atid"]));
            JobInfo objJobInfo = Global.ObjJobController.GetJobInfo(objActiveTestInfo.JobInfoId);
        ...
        protected void printTestList_SelectedIndexChanged(object sender, EventArgs e)
        {
            //selectedTest = printTestList.SelectedValue;
        }
